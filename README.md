...
####  Stage A Modified File
 ...
 Shows what you changed, but have not staged.
...
```
Shows what has been staged, but not committed. 
...
1. commit # Opens the editor we defined above or vi

3. In vi click [ESC] i to enter insert mode

5. Type a heading that briefly explains the changes in 50 characters or less

7. Describes the original problem that is being addressed

9. Describes the specific change being made

11. Describes the result of the change

13.  Describes any future improvements

15. Post a closes bug notation Closes-Bug: #1291621

17. Hit [ESC] and type wq to save and exit

You can apply this to any editor of your choice.
...
```
Shows that the file was deleted:
...
Keep file, but remove from staging area:
...
```bash
git mv DeleteMe.txt Delete.txt 

```
#### Log Commit History
Shows all of the previous commit messages in reverse order:
```bash
git log 
```
Shows commits on one line:
```bash
...
```bash
git log --author="Derek Banas" 
```
Changes made before this date
```bash
git log --before="2014-04-13" 
```
#### Undoing a Commit
a.If you want to change your previous commit
```bash
git commit --amend  
